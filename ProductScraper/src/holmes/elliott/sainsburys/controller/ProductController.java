...
public class ProductController {
	...
	/**
	 * Method to handle each line of the HTML We could use a HTML parser and DOM
	 * traversing / search, but this can be slow and as we are going through
	 * each line of the HTML we may as well process as we go.
	 * 
	 * @param lineStr
	 *            - The line of HTML
	 */
		...
		if (!productsComplete) {
			if (productMode) {
				...
				if (lineStr.contains("<li>")) {
					...
					String subProductUrl = lineStr.substring(lineStr.indexOf("<a href=") + 9,
							lineStr.lastIndexOf("\""));
					...
					// handle the price line (even though the &pound entity is
					// not finished with a ;)
					...
					// because we are in the products list we must have reached
					// the end
					productMode = false;
		...
		if (!descriptionFound) {
			if (intoDescription) {
				...
				if (lineStr.toLowerCase().contains("</div>")) {
					...
					// if the line is not a starting div then strip the HTML
					// tags and put them into the description
					getUrlResponseObject()
							.setDescription(getUrlResponseObject().getDescription() + cleanHtmlTags(lineStr));
				}
			}
